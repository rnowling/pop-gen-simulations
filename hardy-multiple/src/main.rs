...
type Locus = u8; // each allele had a particular locus is represented as an integer
type Gamete = Vec<Locus>;
type Chromosome = Vec<Gamete>; // a haploid organism has a single gamete per chromosome, a diploid organism has two gametes per chromosome, and so forth
type Individual = Vec<Chromosome>;

struct ChromosomeArchitecture {
    pub n_loci: usize,
    pub n_alleles_per_locus: Vec<usize>
}

impl ChromosomeArchitecture {
    pub fn new(n_loci: usize, n_alleles_per_locus: &Vec<usize>) -> ChromosomeArchitecture {
...
impl GeneticArchitecture {
    ...
    pub fn add_chromosome(&mut self, n_loci: usize, n_alleles_per_locus: &Vec<usize>) -> () {
...
pub trait IndividualSampler<R: Rng> {
    fn sample(&mut self, genetic_architecture: &GeneticArchitecture) -> Individual;
}

impl Population {
    pub fn new<R: Rng, S: IndividualSampler<R>>(genetic_architecture: &GeneticArchitecture, n: usize, factory: &mut S) -> Population {
        let mut parents: Vec<Individual> = Vec::new();
        let mut children: Vec<Individual> = Vec::new();

        for _ in 0..n {
            parents.push(factory.sample(genetic_architecture));
            children.push(factory.sample(genetic_architecture));
        }
        ...
        for mut child in self.children.iter_mut() {
            ...
            for (i, chromosome) in child.iter_mut().enumerate() {
                ...
                for parent in parents.iter() {
                    ...
                    chromosome.push(parent_chromosome[gamete_range.sample(rng)]);
                }
            }
        }
    }
...
fn main() {
    ...
    genetic_architecture.add_chromosome(2usize, &vec![2usize, 2usize]);
    genetic_architecture.add_chromosome(2usize, &vec![2usize, 2usize]);
